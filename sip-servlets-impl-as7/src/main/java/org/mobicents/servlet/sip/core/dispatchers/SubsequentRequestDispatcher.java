			...
			try {
				...
				try {
					...
					if(proxy != null) {
						...
						//Issue http://code.google.com/p/mobicents/issues/detail?id=2264
						if (finalBranch == null && Request.UPDATE.equals(requestMethod)){
							String requestTag = sipServletRequest.getFrom().getParameter("tag");
							List<ProxyBranch> branches = proxy.getProxyBranches();
							for (Iterator iterator = branches.iterator(); iterator.hasNext();) {
								ProxyBranch proxyBranch = (ProxyBranch) iterator.next();
								String proxyTag = proxyBranch.getRequest().getFrom().getParameter("tag");
								if (proxyTag.equals(requestTag)) 
									finalBranch = (ProxyBranchImpl) proxyBranch;
							}
						}
